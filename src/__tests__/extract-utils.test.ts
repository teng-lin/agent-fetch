...
describe('extract/utils', () => {
  ...
  describe('hasAccessGateIndicators', () => {
    it('returns true for matching indicator', () => {
      expect(hasAccessGateIndicators('Please subscribe to continue reading')).toBe(true);
    });

    it('matches case-insensitively', () => {
      expect(hasAccessGateIndicators('SUBSCRIBE TO CONTINUE')).toBe(true);
    });

    it('returns false for clean content', () => {
      expect(hasAccessGateIndicators('This is a normal article about technology.')).toBe(false);
    });

    it('returns false for null', () => {
      expect(hasAccessGateIndicators(null)).toBe(false);
    });

    it('returns false for empty string', () => {
      expect(hasAccessGateIndicators('')).toBe(false);
    });
  });
