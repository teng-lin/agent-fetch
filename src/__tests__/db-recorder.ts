...
import os from 'os';
import fs from 'fs';
import { resolveSitesJson } from '../sites/site-config.js';
...
/**
 * Load the sites config JSON as a string.
 */
 ...
 * - Network: from AGENT_FETCH_NETWORK env var (e.g. 'home-wifi', 'vpn-us-east')
 * - Preset: TLS fingerprint preset, passed explicitly or from AGENT_FETCH_PRESET env var
 */
export function startTestRun(runTypeOrOptions: string | TestRunOptions = 'fetch'): string {
  ...
  const network = process.env.AGENT_FETCH_NETWORK || null;
  const preset = opts.preset ?? process.env.AGENT_FETCH_PRESET ?? null;
...
export function recordTestResult(result: TestResult): void {
  ...
  db.run(
    ...
    INSERT INTO test_results (
      ...
      extract_strategy, content_length, error_message
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `,
    [
      currentRunId,
