...
async function main() {
  ...
  const fetchRuns = db.exec(
    "SELECT run_id, substr(git_commit,1,7) as git_hash, started_at FROM test_runs WHERE ended_at IS NOT NULL AND run_type = 'fetch' ORDER BY started_at"
  );
  ...
  const siteData = db.exec(
    "SELECT e.site, tr.run_id, e.success, e.word_count, e.latency_ms, e.extraction_method, e.error_message, e.status_code FROM test_runs tr JOIN e2e_runs e ON tr.run_id = e.run_id WHERE tr.ended_at IS NOT NULL AND tr.run_type = 'fetch' ORDER BY e.site, tr.started_at"
  );
